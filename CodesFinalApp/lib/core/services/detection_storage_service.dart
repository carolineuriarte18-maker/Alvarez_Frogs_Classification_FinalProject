...
class DetectionStorageService {
  ...
  /// Incremented whenever records change; useful for UI listeners.
  final ValueNotifier<int> recordsVersion = ValueNotifier<int>(0);
  ...
  Future<void> loadRecords() async {
    ...
    _cachedRecords =
        jsonList.map((json) => DetectionRecord.decode(json)).toList()
  ...
  double getAccuracy(RecordFilter filter) {
    final allRecords = getFilteredRecords(filter);
    if (allRecords.isEmpty) return 0.0;
    final correct = allRecords.where((r) {
      if (r.groundTruthIndex >= 0) {
        return r.isCorrect;
      } else {
        return r.isVerified;
      }
    }).length;
    return correct / allRecords.length;
  }

  /// Get accuracy for a specific class index based on filter.
  double getAccuracyForClass(int classIndex, RecordFilter filter) {
    final classRecords = getFilteredRecords(
      filter,
    ).where((r) => r.predictedIndex == classIndex).toList();
    if (classRecords.isEmpty) return 0.0;

    final correct = classRecords.where((r) {
      if (r.groundTruthIndex >= 0) {
        return r.isCorrect;
      } else {
        return r.isVerified;
  ...
  List<List<int>> buildConfusionMatrix(int numClasses, RecordFilter filter) {
    final matrix = List.generate(numClasses, (_) => List.filled(numClasses, 0));
  ...
  Map<int, int> getDetectionsPerClass(RecordFilter filter) {
    ...
    for (final record in getFilteredRecords(filter)) {
      if (record.predictedIndex >= 0) {
        counts[record.predictedIndex] =
            (counts[record.predictedIndex] ?? 0) + 1;
      }
    }
    return counts;
  }
  ...
  int getIncorrectPredictions(RecordFilter filter) {
    return getFilteredRecords(filter).where((r) {
      if (r.groundTruthIndex >= 0) {
        return !r.isCorrect;
      } else {
        return !r.isVerified;
  ...
  double getErrorRate(RecordFilter filter) {
    final allRecords = getFilteredRecords(filter);
    if (allRecords.isEmpty) return 0.0;
    final errors = allRecords.where((r) {
      if (r.groundTruthIndex >= 0) {
        return !r.isCorrect;
      } else {
        return !r.isVerified;
    ...
    return errors / allRecords.length;
  }
  ...
  List<ClassAccuracyStats> getHardestClasses(
    RecordFilter filter,
    int numClasses,
  ) {
    ...
    for (int i = 0; i < numClasses; i++) {
      final classRecords = getFilteredRecords(
        filter,
      ).where((r) => r.predictedIndex == i).toList();
  ...
  List<DetectionRecord> getAdvancedFilteredRecords({
    ...
    return _cachedRecords.where((r) {
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        ...
        final matchesGroundTruth = r.groundTruthClass.toLowerCase().contains(
          query,
        );
  ...
  String exportToCsv({RecordFilter filter = RecordFilter.all}) {
    ...
    // Header
    buffer.writeln(
      'ID,Timestamp,Ground Truth,Ground Truth Index,Predicted,Predicted Index,Confidence,Is Correct,Is Verified',
    );
